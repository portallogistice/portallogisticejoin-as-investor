    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm">
      ...
      <div className="flex items-center gap-4 mb-4">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-gray-500 dark:text-gray-400">المستحق</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded bg-amber-500" />
          <span className="text-xs text-gray-500 dark:text-gray-400">المستلَم</span>
        </div>
      </div>

      <div className="space-y-4">
        {chart.map((item) => {
          const duePct = ((item.due || 0) / maxVal) * 100;
          const receivedPct = ((item.received || 0) / maxVal) * 100;
          const rate = item.due ? ((item.received / item.due) * 100).toFixed(0) : 0;
          ...
          return (
            <div key={`${item.year}-${item.month}`} className="space-y-1.5">
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-400 dark:text-gray-500">
                    {fmtSAR(item.received)} / {fmtSAR(item.due)} ر.س
                  </span>
                </div>
              </div>
              <div className="h-8 bg-gray-100 dark:bg-gray-800 rounded-xl overflow-hidden relative">
                ...
                >
                  {receivedPct > 18 && (
                    <span className="text-white text-xs font-bold">{fmtSAR(item.received)}</span>
                  )}
                </div>
              </div>
            </div>
