...
export function RejectModal({ req, onClose, onSuccess, isLoading, error }) {
  const [notes, setNotes] = useState('');

  const handleSubmit = () => {
    onSuccess({ requestId: req.id, notes });
  };
      ...
      >
        <div className="flex items-start justify-between gap-2 p-4 sm:p-5 border-b border-gray-100 dark:border-gray-700 shrink-0">
          <div className="min-w-0 pr-2">
            <h3 id="reject-modal-title" className="text-base font-bold text-gray-900 dark:text-white">
              رفض الطلب
            </h3>
            <p className="text-xs text-gray-600 dark:text-gray-400 mt-1 break-words">
              طلب: <strong className="text-gray-800 dark:text-gray-200">{req.type_label || req.type}</strong> —{' '}
              {req.full_name}
            </p>
          </div>
          <button
        ...
        <div className="p-4 sm:p-5 space-y-4 overflow-y-auto flex-1 min-h-0">
          <div>
            <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5" htmlFor="reject-notes">
              سبب الرفض (اختياري)
            </label>
            <textarea
              id="reject-notes"
              ...
              disabled={isLoading}
              className="w-full min-h-[5.5rem] px-3 py-2.5 rounded-lg bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-600 text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none resize-y transition-all disabled:opacity-50"
            />
          </div>
        ...
        <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 p-4 sm:p-5 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80 shrink-0">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full sm:w-auto min-h-[44px] px-5 py-2.5 sm:py-2 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 bg-red-600 text-white hover:bg-red-700 shadow-sm transition-all disabled:opacity-50 touch-manipulation"
          >
            {isLoading ? (
              <>
